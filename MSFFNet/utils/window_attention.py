...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # 初始化一个可学习的参数表作为相对位置偏置表，是一个尺寸为((2*Wh - 1) * (2*Ww - 1), num_head)的全零张量
        # 相对位置偏置表中的值表示窗口内任意两个像素相对位置对每个注意力头的偏置值
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH
        # print(self.relative_position_bias_table.shape)

        # get pair-wise relative position index for each token inside the window
        # 为窗口内每对像素构建一个唯一的索引，用于从相对位置偏置表中查找相应的偏置
        # coords_h为窗口高度方向的像素索引，tensor(0, 1, ... , Wh-1)
        coords_h = torch.arange(self.window_size[0])
        # coords_w为窗口宽度方向的像素索引，tensor(0, 1, ... , Ww-1)
